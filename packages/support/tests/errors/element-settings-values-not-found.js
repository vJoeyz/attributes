...
fixture`Element setting value not found`
  .page`http://localhost:3000/packages/support/public/scenarios/errors/element-setting-value-not-found.html`;
...
test('Setting is found - in duplicated element but not directly in exact being tested', async (t) => {
  ...
  await selectItem('element-click-text');

  await clickRunCheck();

  await assertErrorsCountOnReport(1);

  await assertErrorIsOnReport('element-click-text', 'attribute-value-not-found');
  await assertErrorIsOnAttribute('element-click-text', 'attribute-value-not-found');

  await writeFileValidationMessage(
    'Option attribute found in duplicate element but missing in expected one',
    'element-click-text'
  );
});
