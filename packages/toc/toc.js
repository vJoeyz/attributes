(() => {
  ...
  var identifyHeadingElement = (headingElement) => {
    if (headingElement.id)
      return headingElement.id;
    const { textContent } = headingElement;
    if (!textContent)
      return;
    const id = (0, import_slugify.default)(textContent, { lower: true });
  ...
  var collectLinksData = (linkTemplate) => {
    ...
    const collectLinkData = (referenceNode) => {
      ...
      const followingTextNodes = [...component.querySelectorAll(`* ${getSelector2("element", "link")}`)];
      ...
      if (followingComponent) {
        const anchor2 = new Comment(ANCHOR_SELECTOR);
        component.insertBefore(anchor2, followingComponent);
