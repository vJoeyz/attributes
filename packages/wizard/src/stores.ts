...
export const schemaInstances = writable<number>(loadStore<number>(WIZARD_INSTANCES, 1));

schemaInstances.subscribe((instances: number) => {
  persistStore<number>(WIZARD_INSTANCES, instances);
})
/**
 * Walkthrough mode
 */
 ...
 */
export const schemaSettingsInstance = writable<number>(loadStore<number>(WIZARD_SELECT_INSTANCE, 1));
...
schemaSettingsKey.subscribe((key: string | null) => {
  ...
  if (key) {
    persistStore<string | null>(WIZARD_KEY, key);
  }
})
...
schemas.subscribe((schemasAttributes) => {
  if (schemasAttributes.length > 0) {
    schemaSettingsKey.set(loadStore<string | null>(WIZARD_KEY, null));
  }
})
...
export const schemaForm = writable<SchemaInput[]>(loadStore<SchemaInput[]>(WIZARD_FORM, []));


/** Wizard - Reset button show when dirty */
...
schemaForm.subscribe((form: SchemaInput[]) => {
  ...
  persistStore<SchemaInput[]>(WIZARD_FORM, formWithoutValidations);
})
