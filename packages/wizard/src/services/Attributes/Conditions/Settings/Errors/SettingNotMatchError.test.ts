...
describe('Test Error - Attribute condition setting not match', () => {
  ...
  test('Attribute type element not match required setting', () => {
    ...
    .toEqual(
      'The attribute fs-cmsload-element="page-button" is missing the required settings fs-cmsload-mode="pagination" in the fs-cmsload-element="list". Change fs-cmsload-mode="load-under" to fs-cmsload-mode="pagination".'
    )
  });
});
